//
// Copyright (c) 2011-2012, QUALCOMM Corporation. All rights reserved.
//
...
Device (ADCM)
{
	...
	Method (ACDB)
	{
		Name (CFG, Package()
		{
			Package()
			{
				0x1000,      // ACDB Data Buffer Size
				0x60,        // audio_device_id_max_num   
				Package()    // ACDB Meta Data Package
				{
					0x1,     // DSP key for Dolby PP filters
				},
                                0x0,         //PVC support status.1-supported 0-not supported
			},
		})
		RETURN(CFG)    
	}
	...
	METHOD(CPMR)
	{
		Name(CFG, Package()
		{
			...
			Package()
			{
				...
				0,          // Number of Buffer allocation types
			},
		})
		Return (CFG)
	}
...
Device (AUDD)
{
	...
	Method(CIPI)
	{
		Name(CIPD, Package()
		{
			...
			0,	//IHGPIO ,0 
			0,	//mclk_speed ,0 MCLK_SPD_9_6_MHZ
				// 1 for MCLK_SPD_12_288_MHZ
			0,  //vdd_buck ,0 for VDD_BUCK_1P8V 
				// 1 for VDD_BUCK_2P15V
				// 2 for VDD_BUCK_1P8V_2P15V
	...
	Method(RPNF)
	{
		Name(REGV, Package()
		{
			...
			Package() { 0x07702000,   	0x200000,  	0x200000, 0 , 0}, //For Internal codec value: 0x200000, external codec value: 0x00020002, WSCodecMaster value: 0x00000000
		})
		Return(REGV)
	}
	...
	Method(PPNF)
	{
		...
		Name(PP, Package()
		{
			...
			Package()
			{
				...
				Package(){  0, Package(){0, 0, 1}, Package(){3, 21, 1} }, 
			},
...
Device (MBHC)
{
	...
	Method (_CRS, 0x0, NotSerialized) 
	{
		Name (RBUF, ResourceTemplate ()
		{
			//GPIO120 -- US/EU GPIO pin 
			GpioIo(Exclusive, PullDown, 0, 0, , "\\_SB.GIO0", ,) {120} 
		})
		Return (RBUF)
	}
	...
	Method(GPNF)
	{
		...
		Name(GPIO, Package()
		{
			...
			// GPIOUID     Mapped GPIO
			// -------     -----------
			//   1         GPIO_US_EURO_SEL         (GPIO69)

			// Index 0-3 indicates actual index in DSDT table

			Package(){ 1, 0, 0},   //GPIO_US_EURO_SEL  : INITIALVALUE=0 means this write GPIO will be set to LOW  (US Jack Setting); INITIALVALUE=1 means this write GPIO will be set to HIGH (EU Jack Setting)
